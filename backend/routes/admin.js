...
router.post('/exams', async (req, res) => {
  try {
    ...
    const examData = {
      ...
      passingScore: req.body.passingScore !== undefined ? req.body.passingScore : 50,
...
// Update question (partial update) - REFETCH after update
router.patch('/questions/:questionId', async (req, res) => {
  try {
    ...
    // REFETCH questions for the exam to update admin UI
    const examId = question.exam;
    if (examId) {
      const exam = await Exam.findById(examId).populate('questions');
      if (exam) {
        res.json({ question, examQuestions: exam.questions });
        return;
      }
    }
    res.json(question);
...
// AI Generate Questions (add before this)
...
router.get('/exams/:examId/results/csv', async (req, res) => {
  try {
    ...
    results.forEach(result => {
      csv += `${result.student.username},${result.student.email},${result.score},${result.percentage},${result.status},${result.submittedAt},${result.antiCheatingLog.tabSwitches},${result.ipAddress}\n`;
    });
